    ...
    <script src="/layer/layer.js"></script>
        <script type="text/javascript">
                    ...
                    success:function(result){
                        ...
                        if(result.success) {
                            ...
                            $.each(tasks, function(index, task){
                                ...
                                content += '  <td>';
                                content += '      <button type="button" onclick="forwardPage(\''+task.memberid+'\',\''+task.id+'\')" class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
                                content += '  </td>';
                                content += '</tr>';
                            });
                            $("#pdTbody").html(content);
                            
                            $("#Pagination").pagination(taskPage.totalsize, {
            ...
            function forwardPage(memberid,taskid){
            	window.location.href="${APP_PATH}/auth/detail/" + memberid + "/" +taskid;
            }
            function deleteProcDef(id,name){
            	layer.confirm("是否删除该流程信息？",  {icon: 3, title:'提示'}, function(cindex){
            		$.ajax({
                		type : "POST",
                		url : "${APP_PATH}/process/delete/"+id,
                		beforeSend:function(){
                			loadingIndex = layer.load(2, {time: 10*1000});
                		},
                		success : function(result){
    			    		if ( result.success ) {
                            	...
                            	layer.msg("流程信息删除失败", {time:2000, icon:5, shift:6}, function(){
